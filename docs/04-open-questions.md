...
| O-15 | Kolumny stanu przy `dcs.plan_dates` / `dcs.revisions` (`step`): enum Postgres `dcs.step` (jak w projekcie modelu) czy FK do słownika `dcs.dictionaries` typu `workflow_step` (od 1a.07 kroki i statusy obiegu są słownikami, nie enumami)? Enum daje sztywną maszynę stanów i CHECK-i zakresu; FK daje edytowalne etykiety, ale nie wolno pozwolić DC dezaktywować kroku używanego przez maszynę stanów. Zgłoszone 2026-09-04 (DCS 1a.07) | otwarty (do rozstrzygnięcia przed Fazą 1b, nie teraz) | wykonawca + DC | Migracje `dcs.plan_dates` / `dcs.revisions` (1b) |
