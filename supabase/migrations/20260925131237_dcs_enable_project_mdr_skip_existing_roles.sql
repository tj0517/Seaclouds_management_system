-- DCS 1b.24b: "Enable DCS" for a project that already has a team.
--
-- dcs.project_roles hangs on the project, not on enabled DCS (see its own
-- table comment) — the team panel (apps/dcs/lib/project-roles.ts) can staff a
-- project before DCS is ever turned on. The 1b.24 wizard did not account for
-- that: it showed an empty team step, and dcs_enable_project_mdr's
-- project_roles insert had no ON CONFLICT, so repeating an already-assigned
-- (project_id, user_id, role) pair failed the UNIQUE index (23505) and rolled
-- back the whole call, including the mdr_settings insert
-- (supabase/tests/dcs_enable_project_mdr.test.sql section 9 proves this red
-- against the migration below).
--
-- Fix, in one line: ON CONFLICT (project_id, user_id, role) DO NOTHING on
-- that insert. A pair already on the project is skipped, not rejected; new
-- pairs are still inserted and still audited (audit_project_roles() is AFTER
-- INSERT — a skipped row fires no INSERT, so it writes no audit row either,
-- which is exactly "existing roles get no new audit row" without a separate
-- check). An unknown user_id or a role outside dcs.project_role still fails
-- the FK / enum cast before ON CONFLICT is ever evaluated, so those RED
-- proofs (test file, section 8) stay green.
--
-- create or replace, not a drop+create: same signature, same grants (which
-- survive replace regardless — repinned below anyway, matching this
-- function's own established convention). Never edit 20260925111841 itself
-- (CLAUDE.md: a fix is a new migration).

create or replace function public.dcs_enable_project_mdr(
  p_project_id          uuid,
...
begin
  ...
  -- One row per (user, role) pair, exactly like dcs_create_project_mdr, with
  -- one difference (DCS-1b.24b): dcs.project_roles hangs on the project, not
  -- on enabled DCS, so the team panel (apps/dcs/lib/project-roles.ts) can
  -- already have written rows here before DCS is ever enabled. A pair the
  -- wizard's payload repeats is therefore not a conflict to reject — ON
  -- CONFLICT DO NOTHING skips it instead of raising 23505 and rolling back
  -- the mdr_settings insert above with it. A skipped pair fires no INSERT,
  -- so audit_project_roles() (AFTER INSERT, read on prod 2026-09-25) writes
  -- no row for it — "existing roles get no new audit row" falls out of this
  -- for free, not from an extra check. An unknown user_id or a role outside
  -- dcs.project_role still fails the FK / enum cast before ON CONFLICT is
  -- ever evaluated, so those refusals are unchanged. An empty p_roles (no
  -- DC) is accepted here too — the wizard is where "at least one DC" is
  -- enforced (task decision, tj 2026-09-25), not the database, matching how
  -- dcs_create_project_mdr already treats "no DC" as a warning rather than a
  -- block.
...
comment on function public.dcs_enable_project_mdr(
  ...
  'DCS 1b.24/1b.24b: enables DCS on an EXISTING project — dcs.mdr_settings + '
  ...
  'a (user_id, role) pair already assigned to the project is skipped (ON '
  'CONFLICT DO NOTHING on the project_roles unique key), not rejected — a '
  'project can be staffed before DCS is enabled.';

-- Grants survive CREATE OR REPLACE, but pinned here again anyway, same
-- convention as the migration this one amends.
