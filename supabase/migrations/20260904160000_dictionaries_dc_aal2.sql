-- DCS 1a.11 / O-14: require aal2 (password + TOTP) for DC and admin writes
-- on dcs.dictionaries. Frontend enforcement (proxy.ts redirect to /mfa) is
-- the UX; this migration is the guarantee that survives a direct
-- PostgREST/SQL call bypassing the frontend.
--
...
-- before; the admin ALL policy also gets aal2 so an admin session without a
-- verified second factor can't write either.
...
alter policy "Admins manage dictionaries" on dcs.dictionaries
  using (
